...
const filteredUsers=input ? users.filter((user)=>user.fullName.toLowerCase().includes(input.toLowerCase())):users
  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-l-xl min-w-[250px]
      ...
      <div className='p-5'>
        <div className='flex justify-between items-center'>
          ...
          <div className="relative py-2 group">
            ...
            rounded-md bg-[#282142] border-gray-600 text-gray-100 hidden
            group-hover:block'>
        ...
        px-4 mt-5'>
          ...
          <input on onChange={(e)=>setInput(e.target.value)} type="text" className='bg-transparent border-none outline-none
      ...
      <div className='flex flex-col'>
        {filteredUsers.map((user, index)=>(
          ...
          rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
            ...
            alt="" className='w-[35px] aspect-[1/1] rounded-full' />
            ...
            {unseenMessages[user._id]>0 && (
              <p className='absolute top-4 right-4 text-xs h-5
              w-5 flex justify-center items-center rounded-full bg-violet-500/50'>{unseenMessages[user._id]}</p>
            )}
          </div>
        ))}
      </div>
